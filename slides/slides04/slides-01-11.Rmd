...
output:
  slidy_presentation:
    ...
    incremental: false
    widescreen: true
    footer: "Copyright (c) Prof. Washington S. da Silva"
---
...
- A regressão é provavelmente a ferramenta mais importante à disposição do 
econometrista. 

- Mas o que é a análise de regressão?

- Técnica estatística que busca descrever e avaliar a relação entre uma 
determinada variável (geralmente chamada de variável dependente, $\mathbf{y}$) 
e uma ou mais outras variáveis (geralmente conhecidas como variável(is) 
independente(s), $\mathbf{x}$).
...
- Variável Resposta = $Y$
    ...
    - Em modelos de regressão a variável resposta $Y$ é **aleatória, numérica e 
      contínua**.

    - $Y$ pode ser numérica discreta ou categórica, mas devemos usar uma classe 
      mais ampla de modelos (Modelos Lineares Generalizado).

- Variáveis Explicativas/Preditoras = $X_1,X_2,\ldots,X_k$

    - As variáveis preditoras $X_1,X_2,\ldots,X_k$ sao aleatórias e podem ser 
...
São utilizados para descrever uma relação entre:
	
- Uma variável resposta $Y$ (numérica, contínua) e
	
- Uma ou mais variáveis preditoras $X_1,X_2,\ldots,X_k$
	
    + $k = 1$: Regressão Simples 
    + $k > 1$: Regressão Múltipla   
...
- Para ser linear, um modelo de regressão **deve ser linear em relação aos parâmetros** 
($\beta$). O modelo não precisa necessariamente  ser linear em relação às 
...
## Regressão Múltipla: Álgebra Matricial
...
## Propriedades dos Estimadores ($\hat{\beta}$) de MQO
...
- Precisamos de medidas da precisão da regressão ($\sqrt{\hat{\sigma}^2}$) e 
das estimativas dos parâmetros $V(\hat{\beta})$.



## Estimador da Precisão do Modelo ($\sqrt{\hat{\sigma}^2}$)

- Dadas **H1**, **H2**, **H3** e **H4**, um  estimador não viesado da 
variância ou do **erro padrão** da regressão é:
...
## Testes de Hipóteses/Significância: Teste $F$
...
## Testes de Hipóteses/Significância: Teste $t$
...
-	 **Interpretação:** $R^2$ mede a porcentagem da variação total dos dados ($SQT$) que é 
...
## Etapas Típicas: Análise de Regressão {.small}

1. Prepação dos Dados
2. Análise Exploratória dos Dados
3. Estimação dos parâmetros do modelo completo.
...
## Estimação de Parâmetros e Inferência em R
...
Teste da constância da variância dos resíduos:
...
## Multicolinearidade: Fator de Inflação da Variância
...
$|vif| > 5$ e $|vif| > 10$ são os critérios mais comuns para dizer se uma variável está causando multicolinearidade, ou não. 
